...
describe('Tool Interface', () => {
  it('should define the expected methods in types.d.ts', () => {
    // Read the types.d.ts file content to check the Tool interface
    const typesPath = path.join(process.cwd(), 'types/types.d.ts');
    const typesContent = fs.readFileSync(typesPath, 'utf8');
    
    // Check for Tool interface definition
    assert.ok(typesContent.includes('interface Tool'), 'types.d.ts should define Tool interface');
  ...
  it('should define the expected return types for methods in types.d.ts', () => {
    // Read the types.d.ts file content to check the Tool interface
    const typesPath = path.join(process.cwd(), 'types/types.d.ts');
    const typesContent = fs.readFileSync(typesPath, 'utf8');
